...
if(isset($_GET['id'])){
	...
	$qry = $conn->query("SELECT concat(UPPER(lastname),', ',othernames) as name, contact FROM facilitators where id=".$facilitator_id)->fetch_array();
	foreach($qry as $k =>$v){
		 $$k = $v;
	}
...
<div class="container-fluid">
	...
	<p><b>Facilitator's Name:</b> <?php echo ucwords($name) ?></p>
	<p><b>Contact:</b> <?php echo ucwords($contact) ?> </p>
	<hr>
...
<script>
	$('#edit').click(function(){
		uni_modal('<b>Modify Registered Schedule</b>','manage_schedule.php?id=<?php echo $id ?>','mid-large')
	})
